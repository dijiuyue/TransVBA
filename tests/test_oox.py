...
class TestSetRunFontSize:
    def test_format_all_runs_removes_duplicate_bold_properties(self):
        doc = Document()
        para = doc.add_paragraph("1.1.1 Heading")
        W = "http://schemas.openxmlformats.org/wordprocessingml/2006/main"
        rPr = para.runs[0]._element.get_or_add_rPr()
        b_off = etree.SubElement(rPr, f"{{{W}}}b")
        b_off.set(f"{{{W}}}val", "0")
        etree.SubElement(rPr, f"{{{W}}}b")
        bcs_off = etree.SubElement(rPr, f"{{{W}}}bCs")
        bcs_off.set(f"{{{W}}}val", "0")
        etree.SubElement(rPr, f"{{{W}}}bCs")
        ...
        rPr = para.runs[0]._element.find("w:rPr", NSMAP)
        bold_tags = rPr.findall("w:b", NSMAP)
        bold_cs_tags = rPr.findall("w:bCs", NSMAP)
        assert len(bold_tags) == 1
        assert len(bold_cs_tags) == 1
        assert bold_tags[0].get(f"{{{W}}}val") is None
        assert bold_cs_tags[0].get(f"{{{W}}}val") is None
